...
utils.getTodosInfoInBlocks = {};
utils.getTodosInfoInBlocks['/editTodos.html']=(todoData) => {
  let block = title = "";
  todoData.forEach((element) => {
    title = utils.toS(element.title);
    block += `<div onclick=editTodo(${title})><h2>${element.title}</h2>`;
    block += `<p>${element.description}</p></div>`;
  });
  return block;
}
utils.getTodosInfoInBlocks['/viewTodos.html']=(todoData) => {
  let block = title = "";
  todoData.forEach((element) => {
    title = utils.toS(element.title);
    block += `<div onclick=ViewTodo(${title})><h2>${element.title}</h2>`;
    block += `<p>${element.description}</p></div>`;
  });
  return block;
}
utils.getTodosInfoInBlocks['/deleteTodos.html']=(todoData) => {
  let block = title = "";
  todoData.forEach((element) => {
    title = utils.toS(element.title);
    block += `<div onclick=RemoveTodo(${title})><h2>${element.title}</h2>`;
    block += `<p>${element.description}</p></div>`;
  });
  return block;
}
